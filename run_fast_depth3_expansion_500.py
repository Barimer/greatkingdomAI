...
def run_selfplay_fast_depth3_and_merge(num_new_games, model_path, pilot_npz_path, output_npz_path):
    print(f"\n=== Step 1: Generating {num_new_games} New Self-Play Games with Fast Depth3 (K=8) ===", flush=True)
    ...
    all_results = []
    all_game_ids = []
    all_terminations = [None] * num_new_games
    ...
    pool_args = [(300 + i, model_path) for i in range(1, num_new_games + 1)]
    ...
    with multiprocessing.Pool(processes=num_processes, initializer=init_worker, initargs=(model_path,)) as pool:
        for res in pool.imap_unordered(play_one_selfplay_fast_depth3, pool_args):
            ...
            all_terminations[game_idx - 301] = termination
            ...
            if completed % 50 == 0 or completed == num_new_games:
                elapsed = time.time() - start_time
                print(f"    [Generation Progress {completed:04d}/{num_new_games:04d}] Elapsed: {elapsed:.1f}s | Avg Moves: {total_moves/completed:.1f}", flush=True)
                
    new_states = np.array(all_states, dtype=np.int8)
    new_actions = np.array(all_actions, dtype=np.int8)
    new_players = np.array(all_players, dtype=np.int8)
    new_results = np.array(all_results, dtype=np.int8)
    new_game_ids = np.array(all_game_ids, dtype=np.int32)
    new_terminations = np.array(all_terminations, dtype='U10')
    
    print(f"\n  Loading pilot dataset from {pilot_npz_path}...", flush=True)
    old_data = np.load(pilot_npz_path)
    old_states = old_data['states']
    old_actions = old_data['actions']
    old_players = old_data['players']
    old_results = old_data['results']
    old_game_ids = old_data['game_ids']
    old_terminations = old_data['terminations']
    
    states_arr = np.concatenate([old_states, new_states], axis=0)
    actions_arr = np.concatenate([old_actions, new_actions], axis=0)
    players_arr = np.concatenate([old_players, new_players], axis=0)
    results_arr = np.concatenate([old_results, new_results], axis=0)
    game_ids_arr = np.concatenate([old_game_ids, new_game_ids], axis=0)
    terminations_arr = np.concatenate([old_terminations, new_terminations], axis=0)
    ...
    print("  Reconstructing game sequences to compute dataset uniqueness...", flush=True)
    game_seqs = {}
    for i in range(len(game_ids_arr)):
        gid = game_ids_arr[i]
        act = tuple(actions_arr[i])
        if gid not in game_seqs:
            game_seqs[gid] = []
        game_seqs[gid].append(act)
        
    all_reconstructed_seqs = [tuple(game_seqs[gid]) for gid in sorted(game_seqs.keys())]
    unique_seqs = set(all_reconstructed_seqs)
    unique_ratio = len(unique_seqs) / len(game_seqs)
    duplicate_ratio = 1.0 - unique_ratio
    
    avg_moves = len(states_arr) / len(game_seqs)
    
    print(f"  Dataset merged and saved at {output_npz_path} (Size: {file_size_mb:.2f} MB)", flush=True)
    print(f"  Total Games: {len(game_seqs)} | Total Samples: {len(states_arr)}", flush=True)
    print(f"  Uniqueness Ratio: {unique_ratio*100:.2f}% | Duplication Ratio: {duplicate_ratio*100:.2f}%", flush=True)
    
    return {
        "games": len(game_seqs),
        "samples": len(states_arr),
...
def main():
    print("=================================================================")
    print("GREAT KINGDOM AI - FAST DEPTH3 500 GAMES EXPANSION & EVALUATION")
    print("=================================================================")
    
    model_temp08_path = r"C:\Users\User\source\repos\greatkingdomAI\policy_model_v2_new_temp.pth"
    model_new_path = r"C:\Users\User\source\repos\greatkingdomAI\policy_model_v2_fast_depth3_500.pth"
    pilot_npz_path = r"C:\Users\User\source\repos\greatkingdomAI\data\selfplay_fast_depth3_pilot_300.npz"
    npz_path = r"C:\Users\User\source\repos\greatkingdomAI\data\selfplay_fast_depth3_500.npz"
    report_path = r"C:\Users\User\source\repos\greatkingdomAI\fast_depth3_final_report.md"
    artifact_report_path = r"C:\Users\User\.gemini\antigravity-cli\brain\d152306c-deaf-4e14-93d1-7eee1edc93c2\fast_depth3_final_report.md"
    
    # 1. Generate 200 new games and merge with pilot 300 games
    gen_stats = run_selfplay_fast_depth3_and_merge(200, model_temp08_path, pilot_npz_path, npz_path)
    
    # 2. Train Policy Network V2 with the 500-game dataset
    ...
    # Write Final report
    md = []
    md.append("# Great Kingdom AI - Fast Depth3 500 Games Final Report\n")
    md.append(f"* **측정 시간**: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    md.append(f"* **학습 조건**: Fast Depth3 (K=8) 500판 데이터셋 ({gen_stats['samples']:,} 샘플, {gen_stats['unique_ratio']*100:.2f}% Diversity)\n")
    ...
    md.append("| 평가 대상 (Matchup) | 총 판수 | 새 모델 (Fast Depth3 500판) 승률 | 비고 |")
    md.append("| :--- | :---: | :---: | :--- |")
    ...
    md.append("| 지표 (Metrics) | 기존 최고 모델 (Temp0.8, K=5) | Multi-Teacher 모델 | **신규 Fast Depth3 500판 모델** |")
    md.append("| :--- | :---: | :---: | :---: |")
    ...
    if overall_success:
        ...
        md.append(f"- 고유 기보 비율 {gen_stats['unique_ratio']*100:.2f}%를 자랑하는 Fast Depth3 교사의 500판 데이터셋을 통해 Depth3 상대 승률 **{new_v_d3_rate:.1f}%**를 달성하여 성공 기준(35% 이상)을 넘어섰습니다.")
        md.append("- 이는 기존 다중 교사 혼합이나 결정론적 데이터의 한계를 넘어, 고품질 지도 학습만으로도 강력한 일반화 정책 정책 학습이 가능함을 규명한 성과입니다.")
    else:
        md.append(f"- **결론**: 500판(약 {gen_stats['samples']:,} 샘플) 데이터 규모의 확충에도 불구하고 Depth3 상대 승률이 35% 미만에 머물렀습니다.")
        md.append("- 1000판으로의 추가 확장이 필요하거나, 단일 Fast Depth3 교사의 포석 무작위성만으로는 다양한 패턴에 대한 강인한 일반화 대응을 구축하기에 학습 신호의 다양성 한계나 하이퍼파라미터 튜닝이 추가로 요구됨을 시사합니다.")
