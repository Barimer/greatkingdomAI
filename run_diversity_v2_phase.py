...
# ----------------- Phase 1: 1000 Games Generation with T=1.0 & K=8 (First 10 moves) -----------------
def play_one_selfplay_hybrid_v2(args):
    ...
    while not game.game_over and move_count < max_moves:
        ...
        if move_count == 0:
            ...
            # 10수 이하: Temperature = 1.0, Top-k = 8
            move = find_hybrid_move(game, _worker_model, device, temperature=1.0, top_k=8)
        else:
            # 11수 이후: 결정론 모드 (T=None, K=5)
            move = find_hybrid_move(game, _worker_model, device, temperature=None, top_k=5)
...
def run_selfplay_diversity_v2(num_games, model_path, output_path):
    print(f"--- Generating {num_games} Games with Diverse v2 (T=1.0, K=8, First 10 Moves) ---", flush=True)
    ...
    with multiprocessing.Pool(processes=num_processes, initializer=init_worker, initargs=(model_path,)) as pool:
        for res in pool.imap_unordered(play_one_selfplay_hybrid_v2, pool_args):
            completed += 1
...
def run_retrain_model(npz_path, model_v2_path):
    print("\n--- Retraining Policy Network V2 with Diverse v2 Dataset ---", flush=True)
...
def main():
    print("=================================================================")
    print("GREAT KINGDOM AI - DIVERSITY EXPANSION PHASE V2 PIPELINE")
    print("=================================================================")
    ...
    model_new_path = r"C:\Users\User\source\repos\greatkingdomAI\policy_model_v2.pth" # Save as the final primary checkpoint
    output_npz_path = r"C:\Users\User\source\repos\greatkingdomAI\data\selfplay_diverse_v2_1000.npz"
    report_path = r"C:\Users\User\source\repos\greatkingdomAI\dataset_diversity_v2_report.md"
    
    # 1. 1000 Games Generation with T=1.0 & K=8 (First 10 moves)
    stats = run_selfplay_diversity_v2(1000, model_5000_path, output_npz_path)
    ...
    overall_success = unique_pct >= 20.0 and new_v_d3_rate >= 35.0
    final_verdict_str = "SUCCESS (일반화 다변화 입증)" if overall_success else "FAIL (다양성 부족 또는 품질 저하)"
    
    # Write Final report
    md = []
    md.append("# Great Kingdom AI - Dataset Diversity Phase v2 Report\n")
    md.append(f"* **측정 시간**: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    md.append(f"* **검증 조건**: Temperature=1.0, Top-k=8 (초반 10수) + 중반 이후 결정론 모드 (1,000판)\n")
    
    md.append("## 1. 신규 데이터셋 통계 (Dataset Metrics)")
    md.append(f"* **총 게임 수**: {stats['games']} 판")
    md.append(f"* **총 샘플 수**: {stats['samples']:,} 샘플")
    md.append(f"* **고유 기보 비율**: **{unique_pct:.2f}%** ({int(unique_pct*10):,} / 1000 고유 기보) — **판정: {unique_verdict}**")
    ...
    md.append(f"| **B) Temp=0.8 + Top-k=5** | 10.30% | 89.70% | 30.0% | 과적합 탈피 시작 (Generalized) |")
    md.append(f"| **C) Temp=1.0 + Top-k=8 (신규)** | **{unique_pct:.2f}%** | {stats['duplicate_ratio']*100:.2f}% | **{new_v_d3_rate:.1f}%** | **{final_verdict_str}** |")
    md.append("")
    
    md.append("## 3. 실전 대국 검증 결과 (Matchup Statistics)")
    md.append("| 평가 대상 (Matchup) | 총 판수 | 새 모델 (Temp 1.0) 승률 | 비고 |")
    md.append("| :--- | :---: | :---: | :--- |")
    ...
    if overall_success:
        md.append(f"### 최종 판정: **PASS (일반화 성능 완성 - 다음 로드맵 집행)**")
        md.append(f"- 초반 10수까지의 다양성 확장(T=1.0, K=8)과 중반 이후의 실전적 최적 탐색 결정론의 결합이 극강의 성과를 보였습니다.")
        md.append(f"- 고유 기보 비율이 **{unique_pct:.2f}%**로 크게 도달하였으며, 무엇보다 강력한 Depth 3 탐색기를 상대로 승률 **{new_v_d3_rate:.1f}%**를 기록하며 실질적 지능 일반화(Generalization)가 완료되었습니다.")
        md.append("- 따라서 기존 Depth2 및 구형 Teacher를 전적으로 폐기하고, 본 **V2(Diverse v2) 다양성 가속기를 Teacher로 최종 확정**합니다.")
        md.append("- 다음 로드맵으로 **5000판 대규모 자가대국 데이터셋 수집** 또는 **MCTS & Value Network RL 파이프라인 개발**로 진행을 승인합니다.")
    else:
        md.append(f"### 최종 판정: **FAIL (추가 개선 필요)**")
        md.append("- 고유 기보 비율이나 Depth3 승률이 성공 조건에 미치지 못했습니다. 노이즈 스케일 또는 10수 전환 임계점의 추가 튜닝이 필요합니다.")
        
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("\n".join(md))
        
    print(f"Diversity v2 report successfully saved to: {report_path}", flush=True)
